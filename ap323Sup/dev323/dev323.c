...
06/22/20  FJM  Chnaged ioremap_nocache() to ioremap()
...
static irqreturn_t
ap323_handler(int irq, void *did)
/* ap323_handler( int irq, void *did, struct pt_regs *cpu_regs ) used in older versions of Linux */
{
...
int init_module(void)
{
	...
	for (i = 0, j = 0; i < MAX_APS; i++)
	{
		...
		if (p323)
		{
			...
			ap_address[i] = (unsigned long)ioremap(ap_address[i], 4096);
